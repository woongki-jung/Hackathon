...
Sprint 1~10 전체 구현 완료 (M5 정식 릴리스 달성, 2026-03-15). Sprint 11은 프로덕션 배포 완료 및 검증 스프린트이다.

**Why:** 10 Sprint / 5 Phase 모두 완료. M5(정식 릴리스) 코드/빌드 완료 상태이나, Vercel Postgres 환경변수 미등록으로 인해 실제 프로덕션 배포가 미완료 상태다. Sprint 11은 배포 차단을 해소하고 프로덕션 기능 검증을 완료하기 위한 Post-M5 스프린트이다.

**How to apply:** Sprint 11이 현재 진행 스프린트. 코드 변경보다 환경변수 등록 및 배포/검증 작업이 핵심이다.
...
| Sprint 9 | 구현 완료 (2026-03-15) | 에러 처리 + 접근성 + FTS5 최적화 + 파일 동기화 + 반응형 점검 | docs/sprint/sprint9.md |
| Sprint 10 | 구현 완료 (2026-03-15) | 성능 최적화 + 보안 강화 + 기술 부채 + 배포 + 사용자 가이드 | docs/sprint/sprint10.md |
| Sprint 11 | 계획 완료 (2026-03-15) | 프로덕션 배포 완료 및 검증 (Vercel Postgres 환경변수 등록 + 재배포 + 기능 검증) | docs/sprint/sprint11.md |
...
- 코드 변경이 거의 없는 배포/운영 스프린트 — 환경변수 등록과 기능 검증이 핵심
- P0(배포 차단): POSTGRES_URL + POSTGRES_URL_NON_POOLING Vercel 환경변수 등록 → 재배포
- P1(기능 검증): 로그인, 대시보드, 환경설정(웹훅), 웹훅 수신, 분석 배치, 용어사전 검색 순서로 검증
- P2(선택): 불필요 환경변수 제거(DATABASE_PATH 등), vercel.json Cron Jobs 설정
- 프로덕션 URL: https://domain-dictionary-iota.vercel.app
- Vercel Hobby 플랜: Cron Jobs 1일 최대 2회, Serverless 함수 실행 10초 제한
- 웹훅 수신 아키텍처: POST /api/webhook/<code>로 본문 수신 → analysis_queue에 등록 → /api/mail/check 호출 또는 Cron으로 분석 실행
- instrumentation.ts가 최초 요청 시 실행되어 관리자 계정 자동 생성됨 (DB 마이그레이션 불필요)
